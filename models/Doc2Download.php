...
class Doc2Download
{
    ...
    private function generateDocument($values)
    {
        ...
        $section->addText('допроса подозреваемого', null, 'topP');
        ...
        $table->addRow();
        $table->addCell(2267.71, $cellColSpan1) ->  addText('г. Вологда', null, array('align' =>'left'));
        $table->addCell(7086.61, $cellColSpan2) ->  addText($values['createdate'], null, array('align' =>'right'));    
                
                
        //Текстовка
        $section->addText('Дознаватель УМВД России по Вологодской области ' . $values['rank'] . ' ' . $values['name'] . ' в помещении ' . $values['room'] . ' (г. Вологда, ул.Ленина, д. 15) в соответствии с частю второй ст. 46, ст. 180 и 190 УПК РФ допросил по уголовному делу №' . $values['number'] . ' в качестве подозреваемого:' );
        ...
        $section->addText('Подозреваемый:                                                    __________________________');
