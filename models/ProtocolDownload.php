...
class ProtocolDownload
{
    ...
    private function generateDocument($values)
    {
        ...
        $section->addText('Участвующим лицам объявлено о применении технических средств: ' . $values['hardware']. ' '.$hardwareO,null, array('align' =>'both', 'spaceAfter' =>0));

         $section->addText('Перед началом  допроса  мне  разъяснены права,  предусмотренные частью четвертой ст. 46 УПК РФ: ',null, array('align' =>'both', 'spaceAfter' =>0));
         $section->addText('1) знать, в чем я подозреваюсь, и получить копию постановления о возбуждении против меня уголовного дела,  либо  копию  протокола задержания, либо  копию  постановления  о  применении  ко мне меры пресечения в виде заключения под стражу;',null, array('align' =>'both', 'spaceAfter' =>0));
         $section->addText('2) давать  объяснения  и  показания  по  поводу  имеющегося  в отношении меня подозрения либо отказаться  от  дачи  объяснений  и показаний;',null, array('align' =>'both', 'spaceAfter' =>0));
        $section->addText('3) пользоваться помощью защитника с момента,  предусмотренного п. 2  и  3  части  третьей ст.  49 УПК РФ,  и иметь свидание с ним наедине и конфиденциально до моего первого допроса;',null, array('align' =>'both', 'spaceAfter' =>0));
        $section->addText('4) представлять доказательства;',null, array('align' =>'both', 'spaceAfter' =>0));
        $section->addText('5) заявлять ходатайства и отводы;',null, array('align' =>'both', 'spaceAfter' =>0));
        $section->addText('6) давать  показания  и  объяснения на родном языке или языке, которым я владею;',null, array('align' =>'both', 'spaceAfter' =>0));
        $section->addText('7) пользоваться помощью переводчика бесплатно;',null, array('align' =>'both', 'spaceAfter' =>0));
        $section->addText('8) знакомиться   с    протоколами    следственных    действий, произведенных с моим участием, и подавать на них замечания;',null, array('align' =>'both', 'spaceAfter' =>0));
        $section->addText('9) участвовать с  разрешения  следователя  или  дознавателя  в следственных действиях,   производимых   по   моему   ходатайству, ходатайству моего защитника либо законного представителя;',null, array('align' =>'both', 'spaceAfter' =>0));
        ...
        $section->addText('Мне разъяснено, что в соответствии со ст. 51 Конституции РФ я не обязан свидетельствовать против самого  себя,  своего  супруга (своей  супруги)  и  других  близких  родственников,  круг которых определен п. 4 ст. 5 УПК РФ.',null, array('align' =>'both', 'spaceAfter' =>0));
        if ($values['roleInThis'] == 'подозреваемый'){
            ...
            $section->addText('Подозреваемому ' . $values['suspect'] . ' объявлено, что он подозревается:',null, array('align' =>'both', 'spaceAfter' =>0));
        }
